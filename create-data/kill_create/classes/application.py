...
class application(object):
	def __init__(self):
		...
		self.AUTHORISED_USE_FILE	= os.path.join(self.SOURCE_DIR, "authorised_use.csv")
	...
	def get_duties(self):
		...
		for i in range(first_chapter, last_chapter + 1):
			my_chapter = chapter(i)
			self.content += my_chapter.xml
	...
	def get_authorised_use(self):
		self.d("Getting authorised use")
		self.authorised_use_list = []
		with open(self.AUTHORISED_USE_FILE) as csv_file:
			csv_reader = csv.reader(csv_file, delimiter = ",")
			next(csv_reader, None)
			for row in csv_reader:
				commodity_code 	= row[0]
				self.authorised_use_list.append (commodity_code)
