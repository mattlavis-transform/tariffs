...
class application(object):
	...
	def write_xml(self):
		self.output_filename = os.path.join(self.XML_OUT_DIR,	"mfn.xml")
	...
	def get_measures_components_from_csv(self):
		print ("get_measures_components_from_csv")
		self.measure_component_list = []
		my_file = os.path.join(self.CSV_DIR, "mfn_analysis_components.csv")
		with open(my_file) as csv_file:
			...
			for row in csv_reader:
				if (len(row) > 0):
					...
					if (goods_nomenclature_item_id != "goods_nomenclature_item_id") and (goods_nomenclature_item_id != ""):
						obj = measure_component(goods_nomenclature_item_id, duty_expression_id, duty_amount, monetary_unit_code, measurement_unit_code, measurement_unit_qualifier_code)
						self.measure_component_list.append(obj)

	def get_measures_from_csv(self):
		print ("get_measures_from_csv")
		self.measure_list = []
		my_file = os.path.join(self.CSV_DIR, "mfn_analysis_measures.csv")
		with open(my_file) as csv_file:
			csv_reader = csv.reader(csv_file, delimiter = ",")
			for row in csv_reader:
				if (len(row) > 0):
					measure_type_id				= row[1]
					geographical_area_id		= row[2]
					goods_nomenclature_item_id	= row[3]
					validity_start_date			= row[4]
					regulation_id				= row[7]
					geographical_area_sid		= row[11]
					goods_nomenclature_sid		= row[12]

					if (goods_nomenclature_item_id != "goods_nomenclature_item_id") and (goods_nomenclature_item_id != ""):
						obj = measure(measure_type_id, geographical_area_id, goods_nomenclature_item_id, validity_start_date, regulation_id, geographical_area_sid, goods_nomenclature_sid)
