...
class profile_29000_modification_regulation(object):
	def import_xml(self, app, update_type, oMessage, transaction_id, message_id):
		...
		if update_type == "20":
			...
			cur = app.conn.cursor()
			try:
				cur.execute("DELETE FROM modification_regulations_oplog WHERE modification_regulation_id = %s", (modification_regulation_id,))
				app.conn.commit()
			except:
				g.app.log_error("modification regulation", "D", None, modification_regulation_id, transaction_id, message_id)
			cur.close()
