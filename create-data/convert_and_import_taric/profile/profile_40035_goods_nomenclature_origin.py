...
class profile_40035_goods_nomenclature_origin(object):
	def import_xml(self, app, update_type, oMessage, transaction_id, message_id):
		...
		derived_goods_nomenclature_item_id	= app.getValue(oMessage, ".//oub:derived_goods.nomenclature.item.id", True)
		...
		if update_type == "20":
			...
			cur = app.conn.cursor()
			try:
				cur.execute("""DELETE FROM goods_nomenclature_origins_oplog
				WHERE goods_nomenclature_sid = %s AND derived_goods_nomenclature_item_id = %s""", (goods_nomenclature_sid, derived_goods_nomenclature_item_id,))
				app.conn.commit()
			except:
				g.app.log_error("goods nomenclature origin", "D", goods_nomenclature_sid, goods_nomenclature_item_id, transaction_id, message_id)
			cur.close()
