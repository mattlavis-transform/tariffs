...
def convert(root):
	...
	for oNode in root.findall('.//oub:quota.closed.and.transferred.event/../../oub:record', functions.namespaces):
		...
		sOut += functions.edi(oNode, "oub:quota.closed.and.transferred.event/oub:quota.suspension.period.sid", 8,  "0", functions.datatype.string) + functions.sDivider
		...
		sOut += functions.edi(oNode, "oub:quota.closed.and.transferred.event/oub:suspension.start.date",       8,  "0", functions.datatype.date) + functions.sDivider
		sOut += functions.edi(oNode, "oub:quota.closed.and.transferred.event/oub:suspension.end.date",         8,  "0", functions.datatype.date) + functions.sDivider
		sOut += functions.edi(oNode, "oub:quota.closed.and.transferred.event/oub:description",                 -1, " ", functions.datatype.string) + functions.sDivider
		sOut += "'\n"
