...
class application(object):
	...
	def get_reliefs(self):
		sql = """
		-- Ships (2357 commodities); this is the whole of 117
		SELECT DISTINCT measure_sid, goods_nomenclature_item_id, 'airworthiness' as measure_type FROM ml.v5 m WHERE measure_type_id = '119'
	...
	def cleanse_xml(self):
		...
		#self.table_xml = re.sub("±([0-9]),([0-9]{1,3})", "±\\1.\\i2", self.table_xml, flags=re.MULTILINE)
	...
	def write_file(self):
		...
		###########################################################################
		self.word_filename = os.path.join(self.OUTPUT_DIR, "non-authorised_reliefs.docx")
	...
	def get_special_paragraphs(self):
		# Get airworthiness
		sFileName = os.path.join(self.SOURCE_DIR, "airworthiness.txt")
		self.para_airworthiness = []
		with open(sFileName, "r", encoding="utf-8") as f:
			reader = csv.reader(f, delimiter="|")
			rows = list(reader)
			for row in rows:
				self.para_airworthiness.append(row[0])
