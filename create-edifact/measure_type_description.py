...
def convert(root):
	...
	sRecordCode = ""
	iLocal = 0
	for oNode in root.findall('.//oub:measure.type.description/../../oub:record', functions.namespaces):
		functions.iCount += 1
		iLocal += 1
		...
		sOut += functions.edi(oNode, "./oub:measure.type.id", 1, "0", functions.datatype.string) + functions.sDivider
		sOut += functions.edi(oNode, "./oub:language.id",     2, "0", functions.datatype.string) + functions.sDivider
		sOut += functions.edi(oNode, "./oub:description",    -1, "0", functions.datatype.string) + functions.sDivider
		sOut += "'\n"
