...
def convert(root):
	...
	sRecordCode = ""
	iLocal = 0
	for oNode in root.findall('.//oub:certificate/../../oub:record', namespaces):
		functions.iCount += 1
		iLocal += 1
		...
		sOut += functions.edi(oNode, "oub:certificate/oub:certificate.type.code", 1, "0", functions.datatype.string)
		sOut += functions.edi(oNode, "oub:certificate/oub:certificate.code", 3, "0", functions.datatype.string)
		sOut += functions.edi(oNode, "oub:certificate/oub:validity.start.date", 8, "0", functions.datatype.date)
		sOut += functions.edi(oNode, "oub:certificate/oub:validity.end.date", 8, "0", functions.datatype.date)
		sOut += "'\n"
