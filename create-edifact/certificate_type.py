...
def convert(root):
	...
	sRecordCode = ""
	iLocal = 0
	for oNode in root.findall('.//oub:certificate.type/../../oub:record', functions.namespaces):
		functions.iCount += 1
		iLocal += 1
		...
		sOut += functions.edi(oNode, "oub:certificate.type/oub:certificate.code", 1, "0", functions.datatype.string) + functions.sDivider
		sOut += functions.edi(oNode, "oub:certificate.type/oub:validity.start.date", 8, "0", functions.datatype.date) + functions.sDivider
		sOut += functions.edi(oNode, "oub:certificate.type/oub:validity.end.date", 8, "0", functions.datatype.date) + functions.sDivider
		sOut += "'\n"
